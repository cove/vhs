...
ffmpeg -i "%INPUT%" -vf "yadif=1,crop=in_w:in_h-28" -c:v libx264 -profile:v high -preset slow -crf 16 -c:a aac -movflags +faststart "%BASENAME% Normal.mp4"
call :vhs_info "%BASENAME% Normal.mp4"

REM Small, fast downloading
ffmpeg -i "%INPUT%" -vf "crop=in_w:in_h-28,scale=640:-2" -c:v libx264 -profile:v baseline -preset slow -crf 20 -c:a aac -b:a 48k "%BASENAME% Small.mp4"
